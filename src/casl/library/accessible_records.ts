...
function accessibleBy<T extends AnyMongoAbility>(
  ...
  if (!subjectType) {
    throw new TypeError(
      `Cannot detect subject type of "${baseQuery.model.modelName}" to return accessible records`,
    );
  }
...
export interface AccessibleRecordModel<T, TQueryHelpers = {}, TMethods = {}, TVirtuals = {}>
  extends Model<
    T,
